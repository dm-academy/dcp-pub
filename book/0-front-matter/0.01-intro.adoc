...
..... *Maturation*
[start=10]
. _Architecture and governance_ We need to understand the big picture, reduce technical debt and redundancy, obtain better economies of scale, and cope with external forces (regulators, vendor partners, security adversaries, auditors) increasingly defining our options.
...
The governing thought experiment is, "what would I turn my attention to next as my IT-based concerns scale up?" For example, I think process management correctly comes before project management; projects depend on having some operational foundation, I perceive, while process-driven organizations do not necessarily need projects (and in the product-centered world, some reportedly avoid them).
