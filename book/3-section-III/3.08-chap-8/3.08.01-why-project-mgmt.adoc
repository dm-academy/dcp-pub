...
The above scenario is in decline, and along with it a way of life for many "IT" professionals. The primary reason is Cloud, and in particular Software as a Service. The long hardware acquisition cycle is gone. And new systems do not need to be nearly as precisely "sized," which meant that development could commence without as much up front analysis. Simpler architectures would suffice until real load was proven. It might then be a scramble to refactor software to take advantage of new capacity, but the overall economic effect was positive, as over-engineering and over-capacity are increasingly avoided.
