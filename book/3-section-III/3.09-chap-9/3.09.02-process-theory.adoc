...
However, we encounter a problem with this ordering. It is taking too long, because people are waiting for space to be assigned before doing anything else. This is a well understood problem; processes can branch and run in parallel (you may well be writing code that works this way; human processes also do this):



Similarities to project management - except with project management they only solve this one.


====== Swimlanes

====== A final caution on technique
endless nuances and debates
