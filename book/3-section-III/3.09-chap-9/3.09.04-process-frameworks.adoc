...
In terms of the new digital delivery approaches, there are a number of issues and concerns with frameworks summarized xref:frameworks[above]:
...
The CMMI has evolved since Humphrey's initial vision, but between its  unrealistic emphasis on statistical process control, and the idea that that process optimization is only relevant at the highest maturity, it is (in the view of this author) badly out of step with current digital trends.

The other frameworks do not embrace statistical process control to the same extent as the CMMI. PMBOK suggests that "control charts may also be used to monitor cost and schedule variances, volume, and frequency of scope changes, or other management results to help determine if the project management processes are in control." This also contradicts the insights of empirical process control, unless the project were also a fully defined process -- unlikely from a process control perspective. (<<PMI2013>>, Kindle Locations 4108-4109).
